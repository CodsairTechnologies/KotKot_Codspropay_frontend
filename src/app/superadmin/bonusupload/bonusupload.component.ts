...
@Component({
  ...
  imports: [ReactiveFormsModule, DialogModule, TableComponent,
    ...
    FormsModule],
  templateUrl: './bonusupload.component.html',
  styleUrl: './bonusupload.component.css'
})
export class BonusuploadComponent {
  token: any;
  adminid: any;
  userName: any;
  status: any;
  ...
  selectedIncentiveId: number | null = null;
  ...
  view(item: any) {
    sessionStorage.setItem("incentiveid", item.incentiveid);
    this.router.navigate(['superadmin/viewbonus']);
  }
  ...
  addAttendanceFileFn() {
    ...
    if (this.selectedAttendanceFile) {
      formdata.append('incentives', this.selectedAttendanceFile);
    }
  ...
  setDeleteIds(item: any) {
    this.selectedIncentiveId = item.incentiveid;
  }



  dltAttendanceFn() {
    if (this.selectedIncentiveId === null) {
  ...
  downloadFile(item: any) {
    // 1. Extract the file path directly from the item object
    const filePath: string | undefined = item.file;

    if (!filePath) {
      console.error('Error: File path is missing in the item object.');
      return;
    }

    const fileUrl: string = `${this.apiurl}${filePath}`;
